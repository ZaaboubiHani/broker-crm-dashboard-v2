...
const CommandDelegateTable: React.FC<CommandDelegateTableProps> = ({ suppliers, data, id, isLoading, displayCommand, onHonor, total, size, page, pageChange, }) => {
    ...
    const [switchesEnablers, setSwitchesEnablers] = React.useState(data.map(command => (command?.finalSupplier === undefined || command?.finalSupplier === null)));

    React.useEffect(() => {
        setSupplierIds(data.map(command => command.finalSupplier?._id));
        setSwitchesState(data.map(command => command.isHonored));
        setSwitchesEnablers(data.map(command => (command?.finalSupplier === undefined || command?.finalSupplier === null)));
    }, [data]);
    ...
    const handleSupplierChange = (index: number) => {
        ...
        newSwitchesEnablers[index] = data[index].finalSupplier === undefined || data[index]?.finalSupplier === null;
        ...
        <div id={id} style={{
                    ...
                    (<ScalableTable
                        ...
                        columns={[
                                        ...
                                        <Select
                                            ...
                                            onChange={(event) => {
                                                if (event.target.value === "other") {
                                                    ...
                                                    params.row.finalSupplier = params.row.suppliers?.find((s: ClientEntity) => s._id === event.target.value);
                                                    data[params.row.index].finalSupplier = data[params.row.index].suppliers?.find((s: ClientEntity) => s._id === event.target.value);
                            ...
                            },
                            {
                                ...
                                renderCell(params) {
                                    return (<Switch
                                        key={data[params.row.index].finalSupplier?._id}
                                        disabled={switchesEnablers[params.row.index]}
                                        checked={switchesState[params.row.index]}
            ...
            <Dialog fullWidth={true} maxWidth='sm' onClose={handleClose} open={showDialog} >
                ...
                <div style={{ display: 'flex', justifyContent: 'space-evenly' }}>
                    <DialogTitle sx={{ marginRight: '155px', fontSize: '16px', marginBottom: '0px', padding: '0px', height: '32px' }} >Nom</DialogTitle>
                    <DialogTitle sx={{ marginRight: '55px', fontSize: '16px', marginBottom: '0px', padding: '0px', height: '32px' }}>Spécialité</DialogTitle>
                </div>
                <List>
                    {
