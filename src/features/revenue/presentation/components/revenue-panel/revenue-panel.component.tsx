...
interface RevenuePanelProps {
    ...
    wilayasRevenue: { name: string, totalSales: number, percentage: number, products: { name: string, quantity: number, total: number, percentage: number }[] }[];
...
const RevenuePanel: React.FC<RevenuePanelProps> = ({ showData, total, totalHonored, totalNotHonored, wilayasRevenue, productsRevenue }) => {
    ...
    const productsColumns: GridColDef[] = [
        ...
        {
            field: 'quantity', headerName: 'Quantité', width: 70,
            align: 'center',
            headerAlign: 'center'
        },
        {
    ...
    if (showData) {
        return (
            <div style={{
                margin: '0px 0px 16px 16px', flexGrow: '1', flex: '1', height: '96%', overflowY: 'auto', overflowX: 'hidden', paddingRight: '8px'
            }}>
                <div style={{
                    display: 'flex'
                }}>
                    ...
                    rows={
                        [...wilayasRevenue.map((row, index) => {
                            ...
                            return {
                                id: index,
                                wilaya: row.name,
                                total: row.totalSales?.toLocaleString('fr-DZ', { style: 'currency', currency: 'DZD' }) ?? (0)?.toLocaleString('fr-DZ', { style: 'currency', currency: 'DZD' }),
                                percentage: row.percentage + '%',
