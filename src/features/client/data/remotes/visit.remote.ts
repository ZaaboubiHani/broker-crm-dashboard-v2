...
export default class ClientVisitRemote {
    async getVisits(clientId: string, page: number, size: number, userId?: string): Promise<ResponseEntity> {
        const token = localStorage.getItem('token');
        try {
            var response = await Api.instance.getAxios().post(`/dashboard/clients/history`, {
                limit: size,
                page: page,
                client: clientId,
                user: userId,
