...
const generatePdf = (command: CommandEntity) => {
    ...
    command.products?.map(product => productsData.push({ Nom: product.product.name ?? '', Quantité: `${product.quantity}` ?? '' }));
    ...
    command.suppliers?.map(supplier => suppliersData.push({ Nom: supplier.fullName ?? '', Wilaya: supplier.wilaya?.name ?? '', Commune: supplier.commune ?? '' }));
    ...
    pdf.text(command.motivations?.map((motivation) => motivation.motivation ?? '').join(', ') ?? '', 16, motivationTitleY + 4);
    ...
    pdf.save(`${command?.visit?.client?.fullName}_${formatDateToYYYYMMDD(new Date())}.pdf`);
};
        ...
        return (
            <div style={{ margin: '8px 0px 8px 8px', flexGrow: '1', overflowY: 'scroll', overflowX: 'hidden', paddingRight: '8px', height: '96%' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'end' }}>
                    <h6 style={{ fontSize: '15px', margin: '0px', height: '32px', fontWeight: 'normal' }}>Heure: {formatTime(command.createdAt!)}</h6>
                    <Button style={{ margin: 'none', fontSize: '13px' }} onClick={() => generatePdf(command)} variant="outlined"> <PictureAsPdfIcon sx={{ marginRight: '8px' }} />Télécharger PDF</Button>
                </div>
                ...
                <h4 style={{ fontSize: 15, fontWeight: '400', margin: '0px', height: '32px' }}> Client: {command?.visit?.client?.fullName}</h4>
                <h4 style={{ fontSize: 15, fontWeight: '400', margin: '0px', height: '32px' }}> Localisation: {(command?.visit?.client?.wilaya?.name ?? '') + ', ' + (command?.visit?.client?.commune ?? '')}</h4>
                <h4 style={{ fontSize: 15, fontWeight: '400', margin: '0px', height: '32px' }}> Téléphone 1: {command?.visit?.client?.phoneNumberOne}</h4>
                <h4 style={{ fontSize: 15, fontWeight: '400', margin: '0px', height: '32px' }}> Téléphone 2: {command?.visit?.client?.phoneNumberTwo}</h4>
                <h4 style={{ fontSize: 15, fontWeight: '400', margin: '0px', height: '32px' }}> E-mail: {command?.visit?.client?.email}</h4>
                ...
                <h4 style={{ fontSize: 15, fontWeight: '400', margin: '0px', height: '32px' }}> Potentiel: {command?.visit?.client?.potential}</h4>
                <h4 style={{ fontSize: 15, fontWeight: '400', margin: '0px', height: '32px' }}> Nombre de visites: {command?.visit?.client?.visitsNumber}</h4>
                ...
                <h4 style={{ fontSize: 17, margin: '0px', height: '32px' }}> <InventoryIcon style={{ fontSize: 17 }} /> Produits:</h4>
                {
                    command.products?.map((product) => (
                        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                            ...
                            <h6 style={{ fontSize: 15, fontWeight: '400', margin: '0px', height: '32px' }}>total: {product.total}</h6>
                        </div>
                    ))
                }
                <Divider component="div" style={{ margin: '8px 0px' }} />
